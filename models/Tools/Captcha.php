...
class Captcha
{
    ...
    /**
     * 输出验证码并把验证码的值保存的\Yii::$app->session中
     * 验证码保存到\Yii::$app->session的格式为： array('verify_code' => '验证码值', 'verify_time' => '验证码创建时间');
     * @access public     
    ...
    public function entry($id = '') {
        ...
        // 验证码使用随机字体
        $ttfPath = dirname(__FILE__) . '../../assets/Verify/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
        ...
        $secode     =   array();
        $secode['verify_code'] = $code; // 把校验码保存到\Yii::$app->session
    ...
    private function _background() {
        $path = dirname(__FILE__).'../../assets/Verify/bgs/';
        $dir = dir($path);
